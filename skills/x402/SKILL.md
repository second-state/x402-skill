...
```bash
test -x ./skills/x402/scripts/x402curl
```

**2. Wallet configured:**

```bash
grep -q "X402_PRIVATE_KEY\|X402_WALLET" .env 2>/dev/null
```

If **either** check fails, tell the user:

> x402 is not fully set up. Follow the guide in `skills/x402/Install.md` to install x402curl and configure a wallet.
...
Replace `curl` with `./skills/x402/scripts/x402curl` and adjust flags:
...
# Transformed
./skills/x402/scripts/x402curl -X POST https://api.example.com/endpoint \
  -H "Content-Type: application/json" \
  -d '{"key": "value"}'
```
...
| 5 | Config error | "Wallet configuration error. Follow `skills/x402/Install.md` to fix your setup." |
...
./skills/x402/scripts/x402curl https://paid-api.example.com/data
# → 200 with data
```
...
# 2. Retry
./skills/x402/scripts/x402curl -X POST https://translate.example.com/v1 \
  -H "Content-Type: application/json" \
  -d '{"text": "Hello", "target": "es"}'
...
# 2. Retry
./skills/x402/scripts/x402curl -X POST https://ocr.example.com/extract \
  -F "file=@document.pdf"
```
...
# 2. Retry
./skills/x402/scripts/x402curl -o result.json https://paid-api.example.com/report
```
